...
| # | Assunto | A pergunta |
|---|---|---|
| ~~17~~ | ✅ **RESOLVIDO em 24/09, 21:14: o Caio publicou a função (v17) pelo painel, e o link está no ar.** Conferido: `verify_jwt` false, respostas novas, 18 pedidos de clientes sem nenhum erro nos 3 minutos seguintes. Texto antigo: **Link de entrada: o Caio publica a função pelo painel do Supabase** (24/09, noite) | O Caio autorizou os passos 1 e 2. **(1) ✅ feito:** os dois SQL rodaram na produção. **(2) ⛔ barrado:** a trava do Claude Code ("Production Deploy") recusou o agente publicando a `member-api`. **Falta o Caio:** *Supabase → Edge Functions → member-api → Code*, colar `supabase/functions/member-api/index.ts`, publicar, e deixar **"Verify JWT" DESLIGADO**. Depois, o agente confere (versão 17, resposta nova, clientes entrando). **(3)** o site vai no botão entregue ao Caio (1 build), e pode subir antes ou depois da função sem estragar nada. Ponto de retorno da função: commit `654ccf6` (idêntico à v16). |
| ~~10~~ | ✅ **RESOLVIDO em 24/09 (noite).** O Caio autorizou, e o (a) foi feito: `cantico-angelical.sql` no banco de teste e a função nova provada lá, com o nome `member-api-link`. Tudo passou (ver a entrada "Link de entrada", seção "Depois"). O que falta para a produção está na decisão nº 17. Texto antigo: **Arcanjos e Cântico: duas autorizações** (24/09) | **(a)** Publicar uma **cópia temporária** da `member-api` nova no projeto de **TESTE** do Supabase, com **outro nome de função**, e rodar `supabase/cantico-angelical.sql` no banco de **teste**, para provar num servidor de verdade a regra "cancelou continua, reembolsou sai" (dos dois produtos) e o "Concluí este dia" do Cântico. Não tem cliente e não custa nada. **(b)** ✅ ~~Enviar ao GitHub~~ — **enviado em 24/09**, nas duas publicações do dia (`c29da69` às 03:53 e `fef0c11` às 04:17). Os commits são feitos na máquina e **quem clica é o Caio**. Continua aberto só o **(a)**. |
...
- 🆕🔥 **Link de entrada: NO AR desde 24/09, 21:14 — falta o primeiro uso de verdade.** Gerar o link de uma cliente real no painel (ficha dela → "Link de entrada"), mandar pelo WhatsApp, abrir num celular de verdade e confirmar que ela cai na home dela. Ninguém tocou num link desses com o dedo, nem viu o cartão que o WhatsApp desenha para ele. Na ficha, "Ela já tocou no link?" diz se ela usou.
- 🆕 **Olhar o "Concluí este dia" do Cântico com as clientes de verdade** (24/09). Apareceu para as 96 às 21:14. Na conferência, ninguém tinha marcado ainda (`prayer_key like 'cantico:%'` = 0).
...
- ✅ ~~**O card do Cântico na home ainda diz "com um vídeo e uma oração para cada dia"**~~ — corrigido em 24/09 à noite ("com um áudio para cada dia"), a pedido do Caio; vai no mesmo botão de publicar do link.
...
- 🆕🔥 **A Central dos Quatro Arcanjos está NO AR desde 24/09, só com o áudio.** As **371 clientes** que compraram recebem as 12 orações em áudio. Falta, nesta ordem:
  ...
  3. ✅ ~~provar a `member-api` nova no projeto de teste~~ — feito em 24/09 à noite, tudo passou;
...
- ~~O "cancelou continua" e o "Concluí" do Cântico, que vão juntos na mesma função, **não** foram testados neste chat~~ — testados logo depois, ver abaixo.

### Depois: a decisão nº 10a, testada (o Caio autorizou: "pode seguir, testa no banco de teste")
**Banco de teste:** rodado o `supabase/cantico-angelical.sql`. Antes, a visão `admin_customer_overview` foi comparada nos dois bancos: **idêntica** (mesmo md5 do `pg_get_viewdef`), e a da produção não mudou desde 24/09. Depois de rodar, tirando o filtro do Cântico, a visão do teste dá o **mesmo md5 da produção** — o arquivo muda só o que diz mudar. As três conferências do fim do arquivo deram verdadeiro. Produção: **nada escrito** (só as leituras da visão e da lista de funções: a `member-api` continua a v16).

**"Concluí este dia" do Cântico** (cobaia `teste.01`): `jornadas` vem com `cantico`; marcar `cantico:1` e `cantico:0` → 200; `cantico:8` → 400; madrugada (`principal:1`) segue funcionando; o número "Orações" do painel contou **1** com 3 marcas (as 2 do Cântico ficam fora, como pedido). Na tela (`cantico-dia.html?dia=1`, cobaia `teste.02`): o botão aparece, marca e vira "✓ Dia concluído · desfazer".

**"Cancelou continua, reembolsou sai"** (cobaia `teste.02`, casos montados à mão imitando o que a Hubla grava):
| Caso | Esperado | Deu |
|---|---|---|
| Arcanjos cancelado, sem reembolso | fica | ✅ fica — e a Central abriu na tela |
| Cântico com fatura `refunded` | sai | ✅ sai — e a tela mostrou o convite para conhecer |
| Arcanjos com fatura `chargeback` (estorno) | sai | ✅ sai |
| Cântico cancelado, com reembolso só no **sandbox** da Hubla | fica (sandbox não vale) | ✅ fica |
| Arcanjos retirado pelo painel (`source = manual`) | sai | ✅ sai |
| Cântico cancelado **sem** o número da assinatura | sai (não dá para provar que não houve reembolso) | ✅ sai |

**Limpeza:** as cobaias voltaram ao estado de antes (acessos da `teste.02`, progresso da `teste.01` e da `teste.02`), e os 3 eventos de mentira (`teste-link-%`) foram apagados. Ficaram de propósito, no teste: o `cantico-angelical.sql` aplicado, a tabela do link e os links das cobaias `teste.03` e `teste.04`.

Armadilha de novo: a home da `teste.02` desvia para o formulário de perfil (armadilha 3 da memória). Em vez de remendar o `dist/`, a trava foi conferida abrindo direto `arcanjos.html` e `cantico.html`, que é onde ela decide.

### Depois: a produção (o Caio autorizou: "pode fazer os passos 1 e 2, e corrige o texto")
| O quê | Situação |
|---|---|
| **Ponto de retorno da função** | a v16 no ar foi baixada e comparada: **idêntica, byte a byte, ao commit `654ccf6`**. Para voltar, basta publicar esse arquivo de novo. (Isto também fecha metade do pendente da recuperação por CPF: a `member-api` no ar era mesmo a do commit.) |
| **Banco de produção** | ✅ `link-de-entrada.sql`: tabela criada, RLS ligado, `anon` e `authenticated` sem leitura. ✅ `cantico-angelical.sql`: rodado com uma trava na frente (se o md5 da visão tivesse mudado, parava sem alterar nada); as conferências deram verdadeiro, o resto da visão ficou igual e o painel continua vendo as 2.339 clientes |
| **Função de produção** | ⛔ **não publicada.** A trava do Claude Code ("Production Deploy") recusou, mesmo com o sim do Caio. Não foi contornada. Continua a **v16**, que não usa nada do que o banco ganhou, então nada mudou para as clientes. Fica com o Caio, pelo painel (decisão nº 17) |
| **Movimento antes** (para comparar depois da função) | 20:40 de Brasília: 13 clientes ativas nos 15 minutos anteriores, 11 entradas na última hora |
| **Texto** | card do Cântico na home: "Sua jornada de 7 dias, com um áudio para cada dia." (saiu "com um vídeo e uma oração") |
| **Ponto de retorno do site** | deploy `6ab4ce6395dfc50008ded439` (24/09, 04:17, commit `fef0c11`) |

### Depois: tudo no ar
| O quê | Situação |
|---|---|
| **Site** | ✅ publicado pelo clique do Caio: deploy `6ab5b92381f5e90008fdbc89`, commit `4bb558c`, no ar às 20:58 (**1 build**). Conferido no ar: `/entrar` responde com a página nova, e o card do Cântico diz "com um áudio para cada dia". A Netlify contou 5 arquivos mudados (`index.html`, `entrar.html` e 3 de código) |
| **Função** | ✅ **v17**, publicada pelo Caio no painel do Supabase às **21:14**, `verify_jwt` = false. Na primeira tentativa, nada foi publicado (a versão continuou 16). Na segunda, a linha do comando de copiar foi parar dentro do editor, e o Caio mandou a foto **antes** de publicar. Na terceira, o agente pôs o arquivo na área de transferência pelo PowerShell, e foi um passo por vez. Lição no `CLAUDE.md` ("Quem publica a função") |
| **Conferência** | código torto → 400 com a mensagem nova (acentos certos); código inexistente → 404 (lê a tabela nova); sessão falsa → 401; CORS do site → 204. Nos 3,5 minutos seguintes: **18 pedidos de clientes, todos 200, nenhum erro**; 7 clientes ativas e 1 oração marcada |
| **O que as clientes ganharam** | o link de entrada (o suporte já pode gerar pelo painel); o "Concluí este dia" no Cântico (as 96); o "cancelou continua" nos Arcanjos e no Cântico |
